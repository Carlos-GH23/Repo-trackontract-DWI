...
import {FaFileContract,FaChevronLeft,FaChevronRight,FaUserCircle,FaSignOutAlt} from "react-icons/fa";
import { useNavigate } from "react-router-dom";

const SidebarClient = ({ isCollapsed, setIsCollapsed }) => {
    const location = useLocation();
    const navigate = useNavigate();

    const links = [
        { to: "/cliente/contracts", label: "Contratos", icon: <FaFileContract /> },
        { to: "/cliente/profile", label: "Perfil", icon: <FaUserCircle /> }
    ];
    ...
    return (
        <div className={`bg-gradient-to-br from-[var(--color-degradado-inicio)] to-[var(--color-degradado-fin)] text-white transition-all duration-300 ${isCollapsed ? "w-16" : "w-64"} flex flex-col`}>
            <div className="flex items-center justify-between px-4 h-14 shadow-sm border-b border-white">
                {!isCollapsed && <h1 className="text-lg font-semibold">TrackContract</h1>}
