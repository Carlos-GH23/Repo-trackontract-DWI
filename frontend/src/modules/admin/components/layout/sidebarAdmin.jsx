...
import { FaUsers, FaFileContract, FaTags, FaChevronLeft, FaChevronRight, FaUserCircle, FaHistory } from "react-icons/fa";

const sidebarAdmin = ({ isCollapsed, setIsCollapsed }) => {
    const location = useLocation();

    const links = [
        { to: "/admin/usuarios", label: "Gestión de Usuarios", icon: <FaUsers /> },
    ...
    <nav className="flex flex-col gap-2 p-4">
        {links.map(({ to, label, icon }) => (
            <Link
                key={to}
                to={to}
                className={`flex items-center gap-2 p-2 rounded-md transition bg-white/10 hover:bg-white/20 ${
                    location.pathname === to ? "bg-white/20" : ""
                }`}
            >
                {icon}
